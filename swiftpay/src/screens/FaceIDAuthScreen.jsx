// src/screens/FaceIDAuthScreen.jsx
...
export default function FaceIDAuthScreen({ navigation, route }) {
  ...
  const captureAndVerify = async () => {
    ...
    try {
      ...
      if (json.success) {
        navigation.replace('EmitterEmulatorScreen', { receiverId, amount });
      } else {
        Alert.alert('Face ID', json.message);
      }
    } catch (err) {
      console.error('Erreur vérification FaceID', err);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.faceContainer}>
        <Text style={[styles.title, { color: colors.text }]}>Face ID – Vérification</Text>
      ...
      <ModalMenu
        ...
        items={[{ label:'Annuler', icon:'close-outline', action:()=>navigation.goBack() }]}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  container:     { flex:1 },
  header:        { flexDirection:'row', justifyContent:'center', alignItems:'center', padding:16 },
